 ...
 */
export class AzureDevOpsRepositoryNew implements IRepository {
    private gitClient: GitClient | null = null;
    private pullRequestClient: PullRequestClient | null = null;
    private readonly projectName: string;
    private readonly repositoryId: string;
    private readonly organization: string;
    private readonly azureDevOpsUri: string;
    ...
    public async initialize(): Promise<void> {
        try {
            ...
            const baseClient = new AzureDevOpsApiClient(
                this.organization,
                this.projectName,
                this.accessToken
            );
        ...
        try {
            ...
            for (const change of changes.changeEntries || []) {
                ...
                try {
                    ...
                    const fileContentStream = await this.gitClient.getItemContent(
                        this.repositoryId,
                        filePath,
                        undefined,
                        undefined,
    ...
    public async setPullRequestStatus(status: 'approved' | 'rejected' | 'waiting', description?: string): Promise<void> {
        ...
        try {
            ...
            await this.pullRequestClient.createPullRequestReviewer(
                reviewer,
                this.repositoryId,
                this.pullRequestId,
                reviewerId,
            ...
            await this.pullRequestClient.createPullRequestStatus(
                {
                    state: status === 'approved' ? GitStatusState.Succeeded :
                           status === 'rejected' ? GitStatusState.Failed :
                           GitStatusState.Pending,
